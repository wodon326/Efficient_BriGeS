...
delta1_threshold_arr = [0.970]
data_name_arr = ['da_2k']
for name, delta1_threshold in zip(data_name_arr, delta1_threshold_arr,):
    # 파일 경로 설정 tau x
    # file_path = f'/home/wodon326/project/AsymKD_VIT_Adapter_large/output/{name}/eval_metrics-bfm-ddp.txt'

    # 파일 경로 설정 tau 3
    # file_path = f'output/{name}/eval_metrics-bfm-ddp-tartan-air.txt'
    file_path = '/home/wodon326/project/BriGeS-Depth-Anything-V2/evaluation/output/da_2k/eval_metrics-bfm-ddp-tartan-air.txt'
    # 체크포인트를 저장할 리스트
    checkpoints = []
    ...
    with open(file_path, 'r') as file:
        ...
        for line in file:
            ...
            # delta1_acc 값을 추출합니다.
            if 'accuracy' in line:
                match = re.search(r'accuracy\s*:\s*(\d+\.\d+)', line)
                # print(f'match : {match}')
                if match:
                    delta1_acc = float(match.group(1))
            # print(delta1_acc,current_checkpoint)
            ...
            if delta1_acc is not None:
                if delta1_acc >= (delta1_threshold):
