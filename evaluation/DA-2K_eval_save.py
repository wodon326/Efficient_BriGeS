...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if seg_image is None:
            seg_image = image
        return image, seg_image, labels, img_shape, img_path
...
if "__main__" == __name__:
    ...
    outdir = os.path.join(outdir, model_type)
    os.makedirs(
            outdir,
            exist_ok=True
        )
    cmap = matplotlib.colormaps.get_cmap('magma')
    for images, seg_image, labels, img_shape, img_path in tqdm(data_loader):
        ...
        dir_path = img_path[0].split('/')[1]
        img_name = img_path[0].split('/')[2]
        # print(dir_path)
        os.makedirs(
            os.path.join(outdir, dir_path),
            exist_ok=True
        )
        filename = os.path.join(outdir, dir_path,  img_name)
