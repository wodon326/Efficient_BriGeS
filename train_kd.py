...
from InSDA.BriGeS import BriGeS_DepthAnythingV2
from InSDA.kd_grouped_attn_adapter import kd_grouped_attn_adapter
...
def train(rank, world_size, args):
    try:
        ...
        checkpoint = "InSDA_checkpoints/sam_vit_l_0b3195.pth"
        model_type = "vit_l"
        ...
        for child in segment_anything.children():
            ...
            break
        BriGeS = BriGeS_DepthAnythingV2(ImageEncoderViT = ImageEncoderViT).to(rank)
        ...
        # load model
        AsymKD_naive_kd = kd_grouped_attn_adapter().to(rank)
        ...
        AsymKD_naive_kd.freeze_kd_grouped_attn_adapter_with_kd_style()
        ...
        if rank == 0:
            branch_parameters = count_trainable_parameters(model.module.MLP_layers)
            print(f"Parameter Count: {count_parameters(model) - branch_parameters}")
            print(f"Additional Parameter Count: {count_trainable_parameters(model) - branch_parameters}")
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                with torch.no_grad():
                    teacher_prediction, teacher_features = BriGeS.forward_return_teacher_features(depth_image, seg_image)
                ...
                feat_loss = feature_loss[0] + feature_loss[1] + feature_loss[2] + feature_loss[3]
                loss = loss + feat_loss
                ...
                if rank == 0:
                    ...
                    logger.add_scalar("feature_loss/total_loss", feat_loss.item(), total_steps)
