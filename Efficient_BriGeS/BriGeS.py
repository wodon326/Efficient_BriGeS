...
class BriGeS_DepthAnythingV2(nn.Module):
    def __init__(
        self, 
        ImageEncoderViT, 
        encoder='vitl', 
        ...
        super(BriGeS_DepthAnythingV2, self).__init__()
    ...
    def forward(self, x, y):
        ...
        seg_feature = F.interpolate(seg_features, size=(patch_h*2, patch_w*2), mode='bilinear', align_corners=False)
        seg_feature = F.max_pool2d(seg_feature, kernel_size=2)
        reshaped_seg_feature = seg_feature.reshape(depth_features[0].shape[0], depth_features[0].shape[2], patch_h * patch_w).permute(0, 2, 1)

        Cross_Attention_Features = []
        for Cross_Attention_Blocks, Blocks,depth_feature in zip(self.Cross_Attention_blocks_layers, self.Blocks_layers, depth_features):
        ...
        # return depth, Cross_Attention_Features, reshaped_seg_feature
        return depth
    
    def forward_return_teacher_features(self, x, y):
        patch_h, patch_w = x.shape[-2] // 14, x.shape[-1] // 14
