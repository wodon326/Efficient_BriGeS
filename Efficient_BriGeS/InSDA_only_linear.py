...
class InSDA_only_linear(nn.Module):
    def __init__(
        self, 
        ImageEncoderViT, 
        encoder='vits', 
        ...
        super(InSDA_only_linear, self).__init__()
        print("InSDA_only_linear")
        ...
        self.model_emb_dim = out_channels[3]
        depth = 1
        drop_path_rate=0.0
    ...
    def forward(self, x, y):
        ...
        seg_features = self.ImageEncoderViT(y, self.intermediate_layer_idx[self.encoder])
        B,C,H,W = seg_features.shape
        seg_features = seg_features.reshape(B,C,H*W).permute(0, 2, 1)
        ...
        for MLP_layer in self.MLP_layers:
            seg_feat = MLP_layer(seg_features)
            seg_feat = seg_feat.permute(0, 2, 1).reshape(B,self.model_emb_dim,H,W)
            seg_feat = F.interpolate(seg_feat, size=(patch_h*2, patch_w*2), mode='bilinear', align_corners=False)
            seg_feat = F.max_pool2d(seg_feat, kernel_size=2)
            seg_feat = seg_feat.reshape(depth_features[0].shape[0], depth_features[0].shape[2], patch_h * patch_w).permute(0, 2, 1)
            reshaped_seg_features.append(seg_feat)

        Cross_Attention_Features = []
        for Cross_Attention_Blocks, Blocks,depth_feature, reshaped_seg_feature in zip(self.Cross_Attention_blocks_layers, self.Blocks_layers, depth_features, reshaped_seg_features):
