...
from .dinov2_layers.block import Grouped_Block
...
class kd_grouped_attn_adapter(nn.Module):
    def __init__(
        self, 
        encoder='vits', 
        features=64,
        ...
        super(kd_grouped_attn_adapter, self).__init__()
        print("kd_grouped_attn_adapter")
        ...
        self.MLP_layers = nn.ModuleList([
            Mlp(
                in_features=self.pretrained.embed_dim,
                out_features= 1024,
            ) for _ in range(4)])
    ...
    def forward(self, x):
        ...
        kd_features = []
        for feat, mlp_layer  in zip(residual_features, self.MLP_layers):
            mlp_feat = mlp_layer(feat)
            kd_features.append(mlp_feat)
